...
def colorpicker(label, color, flags, ops):
    ...
    # 改成正确的浮点数范围 (0.0-1.0)
    # col_hues = [
    #     imgui.color_convert_float4_to_u32(imgui.ImVec4(1.0, 0.0, 0.0, 1.0)),  # 红色
    #     imgui.color_convert_float4_to_u32(imgui.ImVec4(1.0, 1.0, 0.0, 1.0)),  # 黄色
    #     imgui.color_convert_float4_to_u32(imgui.ImVec4(0.0, 1.0, 0.0, 1.0)),  # 绿色
    #     imgui.color_convert_float4_to_u32(imgui.ImVec4(0.0, 1.0, 1.0, 1.0)),  # 青色
    #     imgui.color_convert_float4_to_u32(imgui.ImVec4(0.0, 0.0, 1.0, 1.0)),  # 蓝色
    #     imgui.color_convert_float4_to_u32(imgui.ImVec4(1.0, 0.0, 1.0, 1.0)),  # 品红
    #     imgui.color_convert_float4_to_u32(imgui.ImVec4(1.0, 0.0, 0.0, 1.0))   # 红色
    # ]
    col_hues = []
    for i in range(7):
        hue = i / 6.0  # 0.0 到 1.0
        r, g, b = imgui.color_convert_hsv_to_rgb(hue, 1.0, 1.0, 0.0, 0.0, 0.0)
        col = imgui.color_convert_float4_to_u32(imgui.ImVec4(r, g, b, 1.0))
        col_hues.append(col)
...
def color_bar(color,color_hsv,color_rgb,ops):
    ...
    return changed
# 在文件开头确保有这些 import
# 以及你用到的内部函数已经在模块中： brush_value_based_on_mode, set_brush_color_based_on_mode

def color_palette(label, color, backup_color, pre_color, colors):
    """
    更稳健的 color_palette 实现：
    - 在 set_cursor_pos 后立即使用 imgui.dummy 扩展父窗口边界，避免断言
    - 使用 try/finally 保证 begin_group()/end_group() 成对
    - push_style_var/pop_style_var 与 push_style_color/pop_style_color 成对
    """
    g_Gimgui = imgui.get_current_context()
    window = imgui.internal.get_current_window()
    if window.skip_items:
    ...
    # --- 第一组：当前色 / 备选 / 上一次色 ---
    # push style vars (按你原来的数字 ID 保留，但确保 pop_style_var 对应)
    imgui.push_style_var(12, 0)
    imgui.push_style_var(14, imgui.ImVec2(-2, -2))
    imgui.push_style_var(15, imgui.ImVec2(-2, -2))
    # push id to isolate
    imgui.push_id(label + "_main")
    try:
        imgui.begin_group()
        try:
            # 使用 start pos 并立即占位，避免 SetCursorPos 导致父窗口边界未扩展
            start_pos = imgui.ImVec2(imgui.get_cursor_pos().x + 1, imgui.get_cursor_pos().y)
            imgui.set_cursor_pos(start_pos)
            imgui.dummy((1, 1))

            s_size = 20
            flag_local = imgui.ColorEditFlags_.no_drag_drop

            changed = imgui.color_button('##current', imgui.ImVec4(*color, 1.0), flag_local, imgui.ImVec2(s_size * 2.5 - 2, s_size * 1.25))
            if imgui.color_button('##origin', imgui.ImVec4(*backup_color, 1.0), flag_local, imgui.ImVec2(s_size * 1.25, 1.25 * s_size)):
                set_brush_color_based_on_mode(backup_color)
                colors.insert(0, backup_color)
            imgui.same_line()
            prev_col = colors[1] if len(colors) > 1 else pre_color
            if imgui.color_button('##previous', imgui.ImVec4(*prev_col, 1.0), flag_local, imgui.ImVec2(s_size * 1.25, 1.25 * s_size)):
                set_brush_color_based_on_mode(prev_col)
                colors.insert(0, prev_col)
        finally:
            imgui.end_group()
    finally:
        imgui.pop_id()
        imgui.pop_style_var(3)  # 对应上面 push_style_var 三次

    # --- 第二组：滑块（size / strength） ---
    imgui.same_line()
    imgui.push_id(label + "_sliders")
    try:
        imgui.begin_group()
        try:
            # slider 起始位置
            slider_start_pos = imgui.ImVec2(imgui.get_cursor_pos().x - 4, imgui.get_cursor_pos().y + 1)
            slider_start_pos2 = imgui.ImVec2(slider_start_pos.x, slider_start_pos.y + 27)  # strength pos

            imgui.set_cursor_pos(slider_start_pos)
            imgui.dummy((1, 1))  # 必须提交 item 才会扩张父边界

            # 计算 size / strength（你的 helper）
            ...
            progress_saturated = max(0.0, min(progress, 1.0))

            # slider 宽度等
            global slider_width
            slider_width_brush = slider_width - 55

            # style color push (两种颜色)
            ...
            try:
                imgui.progress_bar(size / 200, imgui.ImVec2(slider_width_brush, 20.0), f'R:{size}')
            ...
            # 回到 slider_start_pos 并占位
            imgui.set_cursor_pos(slider_start_pos)
            imgui.dummy((1, 1))

            # drag_float 使用 no_input（只显示，不允许直接编辑）
            flag_slider = imgui.SliderFlags_.no_input.value
            imgui.push_style_color(imgui.Col_.frame_bg, imgui.ImVec4(1 / 7.0, 0.6, 1, 0))
            imgui.push_style_color(imgui.Col_.frame_bg_hovered, imgui.ImVec4(1 / 7.0, .7, 1, 0))
            imgui.push_style_color(imgui.Col_.frame_bg_active, imgui.ImVec4(1.0, 1.0, 1.0, 0))
            try:
                imgui.set_next_item_width(slider_width_brush)
                chan, size_val = imgui.drag_float('##Radius', size, 1, v_min_r, v_max_r, '', flags=flag_slider)
                imgui.set_cursor_pos(slider_start_pos2)
                imgui.set_next_item_width(slider_width_brush)
                chan2, strength_val = imgui.drag_float('##Strength', strength, 0.005, 0.0, 1.0, '', flags=flag_slider)
            finally:
                imgui.pop_style_color(3)

            # 将变化写回（如果用户拖动）
            if 'size_val' in locals():
                brush_value_based_on_mode(set=True, size=int(size_val))
            if 'strength_val' in locals():
                brush_value_based_on_mode(set=True, strength=strength_val)
        finally:
            imgui.end_group()
    finally:
        imgui.pop_id()
        ...
        try:
            ...
            if num:
                ...
                for i in range(num):
                    ...
                    imgui.push_id(f"palette_{i}")
                    try:
                        if imgui.color_button(f'palette##{i}', imgui.ImVec4(*colors[i], 1.0), imgui.ColorEditFlags_.no_drag_drop, imgui.ImVec2(s_size, s_size)):
