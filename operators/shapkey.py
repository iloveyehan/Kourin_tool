...
#copy from cats
class RenameShapeKeyApplier(bpy.types.Operator):
    ...
    bl_idname = "kourin.shape_key_to_basis"
    bl_label = 'ShapeKeyApplier.label'
    ...
    def execute(self, context):
        ...
        if new_basis_shapekey in keys_relative_recursive_to_new_basis:
            self.report({'ERROR_INVALID_INPUT'}, 'ShapeKeyApplier.error.recursiveRelativeToLoop', name=new_basis_shapekey.name)
            return {'CANCELLED'}
...
class RenameShapeKeyPruner(bpy.types.Operator):

    # Removes shapekeys where no vertex is meaningfully moved
    bl_idname = "kourin.shape_key_prune"
    bl_label = 'ShapeKeyPruner.label'
    bl_description = 'ShapeKeyPruner.desc'
    bl_options = {'REGISTER', 'UNDO', 'INTERNAL'}
    ...
    def execute(self, context):
        ...
        for shape_key in shape_keys:
            ...
            if (np.abs(vertex_locations) < RenameShapeKeyPruner.TOLERANCE).all():
        ...
        if (len(shape_keys_to_delete) > 0):
            report_messages.append('ShapeKeyPruner.removedPrefix')
